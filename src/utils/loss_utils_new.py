...
def get_rendering_loss(args, model, rgb, acc, gt_rgb, bg_color, extras, time_locate, global_step, target_mask = None):
    ...
    if w_smoke_inside_sdf > 1e-8 and not model.single_scene:
        ...
        inside_sdf = args.inside_sdf * args.scene_scale
        ...
        if global_step > args.uniform_sample_step and args.cuda_ray:
            ...
            # weight_mask: [0,1] for points inside sdf
            # weight_mask_on_dynamic = (( - static_sdf_on_dynamic.detach()) / (inside_sdf + 1e-6)).clamp(0.0, 5.0)
            weight_mask_on_dynamic = (( - static_sdf_on_dynamic.detach()) / (inside_sdf + 1e-6)).clamp(0.0, 1.0)


            smoke_inside_loss_on_dynamic = torch.sum((smoke_den_on_dynamic * weight_mask_on_dynamic) ** 2)
            ...
            # weight_mask = (( - static_sdf.detach()) / (inside_sdf + 1e-6)).clamp(0.0, 5.0)
            weight_mask = (( - static_sdf.detach()) / (inside_sdf + 1e-6)).clamp(0.0, 1.0)

            # smoke_inside_sdf_loss = torch.sum((smoke_den*inside_mask) ** 2) / (inside_mask.sum() + 1e-5)
            smoke_inside_sdf_loss = torch.sum((smoke_den * weight_mask) ** 2)
...
def get_velocity_loss(args, model, training_samples, training_stage, trainVel, global_step):
    ...
    if not model.single_scene:
        ...
        boundary_sdf = args.inside_sdf * args.scene_scale
        # boundary_mask = torch.abs(_sdf) < boundary_sdf
        # inside_weight_mask = (( - _sdf) / (boundary_sdf + 1e-6)).clamp(0.0, 5.0)
        inside_weight_mask = (( - _sdf) / (boundary_sdf + 1e-6)).clamp(0.0, 1.0)
        boundary_weight_mask = ((boundary_sdf - _sdf) / (boundary_sdf + 1e-6)).clamp(0.0, 1.0)

        # boundary_vel = _vel * boundary_mask
        boundary_vel = _vel * boundary_weight_mask
        ...
        boundary_loss = torch.sum(boundary_vel_project2normal ** 2)
        ...
        inside_loss = torch.sum((_vel * inside_weight_mask) ** 2)
