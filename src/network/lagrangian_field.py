...
def create_model(args):
    ...
    vel_model = VelocityNetwork(feature_map, position_map)
    map_model = MappingNetwork(feature_map, position_map)

    
    all_models = {}
    all_models['vel_model'] = vel_model
    all_models['map_model'] = map_model

    grad_vars = []
    for model_name, model in all_models.items():
        grad_vars += list(model.parameters())
        # todo:: check whether repeat adding parameters
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        map_model.load_state_dict(ckpt['map_model'])

    return all_models, optimizer, start


def create_density_model(args, feature_map=None):
    if feature_map is None:
        feature_map = FeatureMapping()
 
    density_map = DensityMapping()

    density_model = DensityNetwork(feature_map, density_map)
    
    all_models = {}
    all_models['density_model'] = density_model
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        # Load model
        density_model.load_state_dict(ckpt['density_model'])

    return all_models, optimizer, start


def save_models(args, i, all_models, optimizer, global_step):
    basedir = args.basedir
    expname = args.expname
    path = os.path.join(basedir, expname, '{:06d}.tar'.format(i))
    torch.save({
        'global_step': global_step,
        'vel_model': all_models['vel_model'].state_dict(),
        'map_model': all_models['map_model'].state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    }, path)

    print('Saved checkpoints at', path)

def save_density_models(args, i, all_models, optimizer, global_step):
    basedir = args.basedir
    expname = args.expname
    path = os.path.join(basedir, expname, '{:06d}.tar'.format(i))
    torch.save({
        'global_step': global_step,
        'density_model': all_models['density_model'].state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    }, path)
