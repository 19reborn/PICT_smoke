...
// from `https://github.com/helpking/UnityPlugins/blob/bf80e5ba082ca8544dc1d2d2111ddbb2cb10ead2/Assets/Packages/Dynamic/Terrains/Morton/UtilsMorton.cs#L19`
...
// from `https://github.com/helpking/UnityPlugins/blob/bf80e5ba082ca8544dc1d2d2111ddbb2cb10ead2/Assets/Packages/Dynamic/Terrains/Morton/UtilsMorton.cs#L19`
inline __host__ __device__ uint32_t __morton2D_invert(uint32_t iValue)
{
...
// coords: int32, [N, 3]
// indices: int32, [N]
__global__ void kernel_morton2D(
    const int * __restrict__ coords,
    const uint32_t N,
    int * indices
) {
    ...
    // locate
    coords += n * 2;
    indices[n] = __morton2D(coords[0], coords[1]);
}


void morton2D(const at::Tensor coords, const uint32_t N, at::Tensor indices) {
    static constexpr uint32_t N_THREAD = 128;
    kernel_morton2D<<<div_round_up(N, N_THREAD), N_THREAD>>>(coords.data_ptr<int>(), N, indices.data_ptr<int>());
}


// indices: int32, [N]
// coords: int32, [N, 3]
__global__ void kernel_morton2D_invert(
    const int * __restrict__ indices,
    const uint32_t N,
    int * coords
) {
    ...
    coords[0] = __morton2D_invert(ind >> 0);
    coords[1] = __morton2D_invert(ind >> 1);
}


void morton2D_invert(const at::Tensor indices, const uint32_t N, at::Tensor coords) {
    static constexpr uint32_t N_THREAD = 128;
    kernel_morton2D_invert<<<div_round_up(N, N_THREAD), N_THREAD>>>(indices.data_ptr<int>(), N, coords.data_ptr<int>());
}
...
// bitfield: uint8, [N]
template <typename scalar_t>
__global__ void kernel_packbits_triplane(
    const scalar_t * __restrict__ grid,
    const uint32_t N,
    const float density_thresh,
    uint8_t * bitfield
...
void packbits_triplane(const at::Tensor grid, const uint32_t N, const float density_thresh, at::Tensor bitfield) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grid.scalar_type(), "packbits", ([&] {
        kernel_packbits_triplane<<<div_round_up(N, N_THREAD), N_THREAD>>>(grid.data_ptr<scalar_t>(), N, density_thresh, bitfield.data_ptr<uint8_t>());
    }));
}
...
__global__ void kernel_march_rays_triplane_train(
    ...
    while (t < far && step < num_steps) {
        ...
        const uint32_t index_yz = 1 * H2_C + level * H2 + __morton2D(ny, nz);
        const uint32_t index_xz = 2 * H2_C + level * H2 + __morton2D(nx, nz);
...
void march_rays_triplane_train(const at::Tensor rays_o, const at::Tensor rays_d, const at::Tensor grid, const float bound, const bool contract, const float dt_gamma, const uint32_t max_steps, const uint32_t N, const uint32_t C, const uint32_t H, const at::Tensor nears, const at::Tensor fars, at::optional<at::Tensor> xyzs, at::optional<at::Tensor> dirs, at::optional<at::Tensor> ts, at::Tensor rays, at::Tensor counter, at::Tensor noises) {
    ...
    rays_o.scalar_type(), "march_rays_triplane_train", ([&] {
        kernel_march_rays_triplane_train<<<div_round_up(N, N_THREAD), N_THREAD>>>(rays_o.data_ptr<scalar_t>(), rays_d.data_ptr<scalar_t>(), grid.data_ptr<uint8_t>(), bound, contract, dt_gamma, max_steps, N, C, H, nears.data_ptr<scalar_t>(), fars.data_ptr<scalar_t>(),
            xyzs.has_value() ? xyzs.value().data_ptr<scalar_t>() : nullptr,
            dirs.has_value() ? dirs.value().data_ptr<scalar_t>() : nullptr,
            ts.has_value() ? ts.value().data_ptr<scalar_t>() : nullptr,
...
void composite_rays_train_forward(const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor ts, const at::Tensor rays, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor weights, at::Tensor weights_sum, at::Tensor depth, at::Tensor image, const bool alpha_mode) {
    ...
    sigmas.scalar_type(), "composite_rays_train_forward", ([&] {
        kernel_composite_rays_train_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), ts.data_ptr<scalar_t>(), rays.data_ptr<int>(), M, N, T_thresh, weights.data_ptr<scalar_t>(), weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), alpha_mode);
    }));
}
...
void composite_rays_train_neus_forward(const at::Tensor alphas, const at::Tensor rgbs, const at::Tensor ts, const at::Tensor rays, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor weights, at::Tensor weights_sum, at::Tensor depth, at::Tensor image) {
    ...
    alphas.scalar_type(), "composite_rays_train_neus_forward", ([&] {
        kernel_composite_rays_train_neus_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(alphas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), ts.data_ptr<scalar_t>(), rays.data_ptr<int>(), M, N, T_thresh, weights.data_ptr<scalar_t>(), weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>());
    }));
}
...
__global__ void kernel_composite_rays_train_hybrid_forward(
    const scalar_t * __restrict__ nerf_sigmas,
    const scalar_t * __restrict__ neus_alpha,
    const scalar_t * __restrict__ nerf_rgbs,
    const scalar_t * __restrict__ neus_rgbs,
    const scalar_t * __restrict__ ts_nerf,  
    const scalar_t * __restrict__ ts_neus,
    const int * __restrict__ rays_nerf,
    const int * __restrict__ rays_neus,
    ...
    while (step < num_steps_nerf + num_steps_neus) {
        ...
        if ((neus_step >= num_steps_neus) || ((nerf_step < num_steps_nerf) && ((float)ts_nerf[0] <= (float)ts_neus[0]))) {
            // nerf
            // const scalar_t alpha = 1.0f - __expf(- nerf_sigmas[0] * ts_nerf[1]); // todo:: check interval
...
// nerf_sigmas, neus_alpha, nerf_rgbs, neus_rgbs, ts_nerf, ts_neus, rays_nerf, rays_neus,
void composite_rays_train_hybrid_forward(const at::Tensor nerf_sigmas, const at::Tensor neus_alpha, const at::Tensor nerf_rgbs, const at::Tensor neus_rgbs, const at::Tensor ts_nerf, const at::Tensor ts_neus, const at::Tensor rays_nerf, const at::Tensor rays_neus, const uint32_t M, const uint32_t M_nerf, const uint32_t M_neus, const uint32_t N, const float T_thresh, at::Tensor weights, at::Tensor weights_sum, at::Tensor depth, at::Tensor image) {
    ...
    nerf_sigmas.scalar_type(), "composite_rays_train_hybrid_forward", ([&] {
        kernel_composite_rays_train_hybrid_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(nerf_sigmas.data_ptr<scalar_t>(), neus_alpha.data_ptr<scalar_t>(), nerf_rgbs.data_ptr<scalar_t>(), neus_rgbs.data_ptr<scalar_t>(), 
                                                                                            ts_nerf.data_ptr<scalar_t>(), ts_neus.data_ptr<scalar_t>(), rays_nerf.data_ptr<int>(), rays_neus.data_ptr<int>(), M, M_nerf, M_neus, N, T_thresh, weights.data_ptr<scalar_t>(), weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>());
    }));
}
...
void composite_rays_train_neus_backward(const at::Tensor grad_weights, const at::Tensor grad_weights_sum, const at::Tensor grad_depth, const at::Tensor grad_image, const at::Tensor alphas, const at::Tensor rgbs, const at::Tensor ts, const at::Tensor rays, const at::Tensor weights_sum, const at::Tensor depth, const at::Tensor image, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor grad_alphas, at::Tensor grad_rgbs) {
    ...
    grad_image.scalar_type(), "composite_rays_train_neus_backward", ([&] {
        kernel_composite_rays_train_neus_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(grad_weights.data_ptr<scalar_t>(), grad_weights_sum.data_ptr<scalar_t>(), grad_depth.data_ptr<scalar_t>(), grad_image.data_ptr<scalar_t>(), alphas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), ts.data_ptr<scalar_t>(), rays.data_ptr<int>(), weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), M, N, T_thresh, grad_alphas.data_ptr<scalar_t>(), grad_rgbs.data_ptr<scalar_t>());
    }));
}
...
__global__ void kernel_composite_rays_train_hybrid_backward(
    ...
    const uint32_t M, const uint32_t M_nerf, const uint32_t M_neus, 
    const uint32_t N, const float T_thresh, 
    scalar_t * grad_nerf_sigmas,
    scalar_t * grad_neus_alpha,
    scalar_t * grad_nerf_rgbs,
    scalar_t * grad_neus_rgbs
) {
    // parallel per ray
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= N) return;
    ...
    if (num_steps_nerf + num_steps_neus == 0 || 
        offset_nerf + num_steps_nerf > M_nerf ||
        offset_neus + num_steps_neus > M_neus ||
        offset_nerf + num_steps_nerf + offset_neus + num_steps_neus > M) return;
    ...
    while (step < num_steps_nerf + num_steps_neus) {
        ...
        if ((neus_step >= num_steps_neus) || ((nerf_step < num_steps_nerf) && ((float)ts_nerf[0] <= (float)ts_neus[0]))) {
        // if ((((float)ts_nerf[0] < (float)ts_neus[0]) && (nerf_step < num_steps_nerf)) || (neus_step >= num_steps_neus)) {
            // nerf
            // const scalar_t alpha = 1.0f - __expf(- nerf_sigmas[0] * ts_nerf[1]);
...
void composite_rays_train_hybrid_backward(const at::Tensor grad_weights, const at::Tensor grad_weights_sum, const at::Tensor grad_depth, const at::Tensor grad_image, 
                                          const at::Tensor nerf_sigmas, const at::Tensor neus_alpha, const at::Tensor nerf_rgbs, const at::Tensor neus_rgbs, const at::Tensor ts_nerf, const at::Tensor ts_neus, const at::Tensor rays_nerf, const at::Tensor rays_neus, 
                                          const at::Tensor weights_sum, const at::Tensor depth, const at::Tensor image, const uint32_t M, const uint32_t M_nerf, const uint32_t M_neus, const uint32_t N, const float T_thresh, 
                                          at::Tensor grad_nerf_sigmas, at::Tensor grad_neus_alpha, at::Tensor grad_nerf_rgbs, at::Tensor grad_neus_rgbs) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_image.scalar_type(), "composite_rays_train_hybrid_backward", ([&] {
        kernel_composite_rays_train_hybrid_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(grad_weights.data_ptr<scalar_t>(), grad_weights_sum.data_ptr<scalar_t>(), grad_depth.data_ptr<scalar_t>(), grad_image.data_ptr<scalar_t>(), 
                                                                                             nerf_sigmas.data_ptr<scalar_t>(), neus_alpha.data_ptr<scalar_t>(), nerf_rgbs.data_ptr<scalar_t>(), neus_rgbs.data_ptr<scalar_t>(), ts_nerf.data_ptr<scalar_t>(), ts_neus.data_ptr<scalar_t>(), rays_nerf.data_ptr<int>(), rays_neus.data_ptr<int>(),
                                                                                             weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), M, M_nerf, M_neus, N, T_thresh, 
                                                                                             grad_nerf_sigmas.data_ptr<scalar_t>(), grad_neus_alpha.data_ptr<scalar_t>(), grad_nerf_rgbs.data_ptr<scalar_t>(), grad_neus_rgbs.data_ptr<scalar_t>());
    }));
}
